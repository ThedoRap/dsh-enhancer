...
/** 8. client-ui-conversation: 入草稿时的 MIME 预校验（文本文件跳过图片校验，否则 .php/.py 在这就被拦）。 */
const CONVERSATION_PREVALIDATE_OLD = 'for (const file of files) imageMediaType(file.type);'
const CONVERSATION_PREVALIDATE_NEW = 'for (const file of files) if (!fileIsTextFile(file)) imageMediaType(file.type);'
/** 9. client-ui-conversation: 不支持的图片类型提示文案（zh/en，原文案只列 4 种格式）。 */
...
export const MEDIA_TYPE_PATCH_SETS = [
  ...
  },
  {
    ...
    patches: [
      ...
      { old: CONVERSATION_PREVALIDATE_OLD, new: CONVERSATION_PREVALIDATE_NEW },
